...
class Xsql {
  ...
  getLimitClause(reqParams) {
    ...
    if ("_size" in reqParams) {
      if (parseInt(reqParams._size) > 0 && parseInt(reqParams._size) <= this.MAX_ROWS) {
        reqParams._len = parseInt(reqParams._size);
      } else if (parseInt(reqParams._size) > this.MAX_ROWS) {
        reqParams._len = this.MAX_ROWS;
      }
    }
